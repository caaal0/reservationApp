...
async function signInWithGoogle(){
  ...
  try{
    ...
    if(response.ok){
      ...
      // console.log(data)
      const userDoc = await fetch(`http://localhost:8080/users/${data.user.uid}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Connection': 'keep-alive',
      ...
      if(userDoc.ok){
        const userDocData = await userDoc.json()
        if(userDocData.success){
          // console.log(userDocData)
          console.log('User document created')
        }else{
          console.log(userDocData.error)
          console.error('Error creating user document | It might already exist')
        }
      }else{
        const userDocData = await userDoc.json()
        console.log(userDocData.error)
